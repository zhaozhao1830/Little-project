...
function bindClickEvent(sModule, sEdit, sLang)
{
    ...
    switch(sModule)
    {
        ...
        case 'education':
            ...
            if(sLang == "c")
            {
                $('#edu_majordes').majorVendorsAssociation({
                    'id'             : 'edu_majordes',
                    'text_id'        : 'edu_majordes',
                    'code_id'        : 'edu_majordes',
                    'association_id' : 'edu_majordes_list',
                    'save_type'      : '1',
                    'float_index_id' : 'edu_major_index',
                    'float_on_id' : 'edu_majordes_div',
                    'language'                : sLang
                });
            }
...
function getEmptyModuleCode(sType)
{
    ...
    aResult['viewresume'] = '<div class="nav_top">'
        ...
        +'</div><div class="h_btm"></div>'
        +'</div></div></div>'
		+'<div class="cup"><a id="resume_switchto">' + lang['resume_switchto'] 
        +'></a></div></div></div>'
		+'<div class="btnbox">'
		+'<span class="p_but" onclick="refreshResume()">' + lang['resume_refresh'] + '</span><span class="p_but geige" onclick="javascript:window.open(\'' + window.cfg.root + '/resume_preview.php?lang=' + $("#isenglish").val() + '&resumeid=' + $("#resumeid").val() + '\')" >' + lang['resume_preview'] + '</span>'
		+'</div>';
...
function showMoreClickEvent(p_oObject)
{
    if($('.main').hasClass('eng')){
        ...
        if( ($(p_oObject).hasClass('on')) && (!$(p_oObject).parent().parent().hasClass('m1')) ){
            $(p_oObject).removeClass('on').children('span').html('更多展开').next().css('display','none').parent().next().css('display','none');
        }else if( (!$(p_oObject).hasClass('on')) && (!$(p_oObject).parent().parent().hasClass('m1')) ){
            $(p_oObject).addClass('on').children('span').html('更多收起').next().css('display','block').parent().next().css('display','block');
        }else if( ($(p_oObject).hasClass('on')) && ($(p_oObject).parent().parent().hasClass('m1')) ){
...
function uploadTypeChange(sType)
{
    ...
    else
    {
        ...
        file.remove(); 
        $("#atta_uploadfile_div").find(".word").html("选择文件");
        showFileName();
